...
class UserService {
  ...
  sendFriendRequest = async (req: Request, res: Response) => {
    const { userId }: SendFriendRequestDTO = req.body;
    const user = req.user;
    if (userId === user._id.toString()) {
      throw new utilities.BadRequestException(
    ...
    const requestAlreadySent = friendRequestsSent.some(
    ...
    const requestAlreadyReceived = friendRequestsReceived.some(
      (requestId) => requestId.toString() === userId
    );
  ...
  acceptFriendRequest = async (req: Request, res: Response) => {
    const { userId }: RespondFriendRequestDTO = req.body;
    const user = req.user;
    const friendRequestsReceived = user.friendRequestsReceived || [];
    const hasRequest = friendRequestsReceived.some(
      (requestId) => requestId.toString() == userId
    );
    if (!hasRequest) {
      throw new utilities.BadRequestException(
  ...
  unfriend = async (req: Request, res: Response) => {
    const { userId }: UnfriendDTO = req.body;
    const user = req.user;
    if (userId === user._id.toString()) {
  ...
  deleteFriendRequest = async (req: Request, res: Response) => {
    const { userId }: DeleteFriendRequestDTO = req.body;
    const user = req.user;
    const friendRequestsSent = user.friendRequestsSent || [];
    const hasRequest = friendRequestsSent.some(
      (requestId) => requestId.toString() == userId
    );
    if (!hasRequest) {
      throw new utilities.BadRequestException(
    ...
    await this.userRepository.update(
      { _id: userId },
      { $pull: { friendRequestsReceived: user._id } }
    );
    return res.status(200).json({
