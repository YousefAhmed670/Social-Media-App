...
class CommentService {
    ...
    updateComment = async (req, res) => {
        ...
        if (updateCommentDto.mentions?.length) {
            validatedMentions = await (0, utilities_1.MentionProvider)(updateCommentDto.mentions, user, this.userRepository);
        }
        const updatedComment = this.commentFactoryService.updateComment(updateCommentDto, commentExists, validatedMentions);
    ...
    freezeComment = async (req, res) => {
        ...
        if (commentExists.userId.toString() !== user._id.toString()) {
            throw new utilities_1.UnauthorizedException("You are not authorized to freeze this comment");
        }
